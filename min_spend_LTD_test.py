...
beta_vec = [0.7, 0.75, 0.8, 0.85, 0.9, 0.953] # impatience
theta = 0.282  # re-entry probability

# NEW PARAMS
gmin_vec = [0.5, 0.6, 0.65, 0.7, 0.725, 0.75, 0.775, 0.8]
tau = 1 # tax rate


# initialize discretized asset values
b_grid = np.linspace(-1.2, 1.2, N)
    ...
    for gmin in gmin_vec:
        ...
        def update_Q(d_choice, P, r):
            ...
            for b in range(N):
                ...
                price_new[y_b_index] = (1 - delta[:,b]) / (1+r)
        ...
        while sup_norm > 0.05:
            ...
            Defaults and Prices
            '''
            price_new, delta = update_Q(d_choice, P, r)
            price_old = price_new
        ...
        print("G-Min=" + str(gmin) + ", Mean Assets:", np.median(b_means))
...
# create percentage values for X and Y axis
gmin_perc_vec = []
for g in gmin_vec:
    gmin_perc_vec.append(g * 100)

b_perc_mat = []
for b_vec in b_mat:
    b_perc_vec = []
    for b in b_vec:
        b_perc_vec.append(b * 100)
    b_perc_mat.append(b_perc_vec)
...
for (i,beta) in zip(range(len(beta_vec)), beta_vec):
    ax1.plot(gmin_perc_vec, b_mat[i], linewidth=3, color=color_list[i], linestyle=lstyle_list[i], label='β=' + str(beta_vec[i]))
plt.title("Savings and Minimum Spending")
plt.xlabel("Minimum Consumption (% of GDP)")
plt.ylabel("Mean Assets (% of GDP)")
plt.legend()
plt.show()
fig.savefig('minspend_vs_savings.pdf', format='pdf', bbox_inches='tight')
